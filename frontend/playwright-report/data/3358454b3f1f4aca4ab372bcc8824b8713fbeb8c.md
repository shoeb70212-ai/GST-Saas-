...
- Name: bank-reconcile.spec.ts >> AI Reconciliation Engine UI >> should display AI match suggestions and allow approval
- Location: e2e\bank-reconcile.spec.ts:43:3
...
```ts
  ...
  3  | 
  4  | test.describe('AI Reconciliation Engine UI', () => {
  5  |   test.beforeEach(async ({ page }) => {
  6  |     const { access_token } = await signUpTestUser();
  7  |     await loginViaSessionInjection(page, access_token);
  8  | 
  9  |     // Intercept backend API calls to avoid hitting real backend during isolated UI tests
  10 |     await page.route('**/api/reconcile/suggestions*', async (route) => {
  ...
  17 |             {
  18 |               id: 'match_123',
  19 |               invoice_id: 'inv_abc',
  20 |               bank_transaction_id: 'txn_xyz',
  21 |               match_type: 'EXACT',
  22 |               allocated_amount: 1000.50,
  23 |               status: 'SUGGESTED',
  24 |               created_by: 'AI',
  25 |               invoices: {
  26 |                 supplier_name: 'Test Supplier',
  27 |                 total_amount: 1000.50,
  28 |               },
  29 |               bank_transactions: {
  30 |                 txn_date: '2026-05-15',
  31 |                 description: 'NEFT Test Supplier',
  32 |                 withdrawal: 1000.50,
  33 |               }
  34 |             }
  35 |           ]
  36 |         })
  ...
  38 |     });
  39 | 
  40 |     await page.goto('/dashboard/reconcile');
  41 |   });
  42 | 
  43 |   test('should display AI match suggestions and allow approval', async ({ page }) => {
  44 |     // Wait for the empty state since we didn't mock the client context selection yet
  45 |     const heading = page.getByRole('heading', { name: /No Client Selected/i });
> 46 |     await expect(heading).toBeVisible();
     |                           ^ Error: expect(locator).toBeVisible() failed
  47 | 
  48 |     // Ideally, we would have a fixture that sets local storage to mock a logged-in state 
  49 |     // and an active client context to test the full table render.
  50 |   });
  51 | });
  52 | 
```